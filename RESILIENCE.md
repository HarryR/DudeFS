...
| **Floor perjury** (attest a floor, receipt beneath it later) | Can let a straggler commit under an already-claimed finality frontier → a verdict some client had called final flips. State still converges (the straggler folds identically for everyone). Proof rides the **artifact issuance chain** (NOTES 43): receipts and watermarks carry a per-signer monotone `issue_seq`, so a WM at seq *s* attesting F plus the same signer's receipt at seq > *s* for an op below F is an ordered, self-contradicting pair — honest nodes cannot produce it (the past gate refuses fresh below-floor acceptances, and re-issues serve the stored original at its original seq), so the proof is frame-free. (The naive WM+receipt pair WITHOUT the seq is *not* proof — an honest node's old receipt beside its newer watermark matches it; that overclaim stood from R1 until finding 17.) Any two distinct artifacts at one `(signer, issue_seq)` — receipt or watermark, any mix — are a FORK-analog proof (the sole exemption: same-op re-issues across epochs), and **gap-free issuance** (the seq ledger commits each position's justification before the deterministic signature, so a crash re-derives the identical artifact instead of burning the seq) means a back-stamped artifact always collides with a genuine occupant. The proof is therefore **complete as well as sound** — no honest node is accusable, no perjurer has a hole to hide in (NOTES 44/45). Preventing (rather than detecting) this needs an honest quorum behind every attestation — exactly the standing trust assumption. |
