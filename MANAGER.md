...
| Audit | `dude verify` | DESIGN §12 / ACCUMULATOR.md | Recomputes the `state_acc` accumulator against the latest checkpoint (O(Δ) with continuity); validates chains and QCs. Any key-holding client can run it — no root key, no window. |
