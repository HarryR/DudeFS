...
> **Status:** companion to [DESIGN.md](DESIGN.md) (rev 5). The question "TLA+ or Lean?" has a structural answer: the design already splits into a coordination layer and a safety layer (DESIGN §8, *safety layering*), and the two layers want different tools. This document states the split, the numbered hypotheses each tool must discharge, the model scope, and how the formal artifacts bind to the eventual implementation.
...
- **A4 — Barrier equivalence.** If cut `C` is final w.r.t. committed set `S`: `fold(S) = fold(snapshot(fold(S≤C)) ∘ tail)`. Snapshot clients and full-history clients are byte-identical — including tombstone death and lineage reset to `(⊥, 0)`.
