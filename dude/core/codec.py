...
def _encode_into(value: object, out: bytearray) -> None:  # noqa: C901
    # A type-dispatch chain: one arm per bencodable shape, plus the rejections that keep the codec
    # canonical. Complexity 12 is the SHAPE of the grammar, not accreted responsibility — splitting
    # it would scatter one closed vocabulary across four functions.
    #
...
def decode(data: bytes | bytearray) -> Bencodable:
    ...
    if isinstance(data, bytearray):
        data = bytes(data)
    # (No `isinstance(data, bytes)` guard: the annotation covers it, and a caller passing
    # anything else is a BUG, not a recoverable condition — dressing it as a CodecError would
    # put a bug inside the expected-error tree. See errors.py.)
...
def _decode_at(data: bytes, pos: int, depth: int = 0) -> tuple[Bencodable, int]:
    ...
    # is outside the DudeError tree and would crash a serving thread. `b"l" * 10**6` from an
...
def _decode_int(data: bytes, pos: int) -> tuple[int, int]:
    # precondition: the caller has checked the tag byte is 'i'
...
def _decode_list(data: bytes, pos: int, depth: int = 0) -> tuple[tuple[Bencodable, ...], int]:
    # precondition: the tag byte is 'l'. Decoded as an immutable tuple (see Bencodable).
    pos += 1
...
def _decode_dict(data: bytes, pos: int, depth: int = 0) -> tuple[dict[bytes, Bencodable], int]:
    # precondition: the caller has checked the tag byte is 'd'
    pos += 1
    ...
    while True:
        ...
        # `depth + 1`, exactly as _decode_list does. Omitting it here reset the counter at every
        # dict level, so nesting through DICTS (`d1:ad1:ad1:a…`) bypassed _MAX_DEPTH entirely and
        # a few-KB frame reached CPython's recursion limit — a RecursionError, which is outside
        # the DudeError tree and therefore an unauthenticated remote process kill under
        # crash-only. Carried over from the previous package as a FIX, not a copy.
