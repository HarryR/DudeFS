...
| the screen tag on every inbound frame | **compared by no layer and no transport.** `crypto.screen_tag` says the receiver keys on its OWN identity and compares, and that this is what makes garbage cost one hash rather than an ECDH — nothing compared, so junk paid for a full sealed-box attempt and a frame tagged for somebody else was opened. Checked in `Postman.deliver` now, which is the one door every carrier comes through: a check living in `InProc` is a check a socket transport would not have |
...
**308 tests, all green.** Lint, format and typecheck clean, six consecutive runs. The long-standing
