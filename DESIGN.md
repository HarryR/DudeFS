...
**Why no guard is ever node-checked — a ruling, not a build artifact (NOTES 49).** The layering is: **nodes decide *who owns* an opaque identity; the fold decides *what that ownership meant*.** Even the write side's currency is fold-judged (a decided-but-stale tag folds `stale`) — so "witness tags for guards" would introduce the system's *first* node-side currency judgment, and it fails three ways before confidentiality is even reached: (1) currency is not derivable from slot state — a node knows only decided-or-not, and making supersession node-visible requires every write to expose its predecessor's tag, leaking the full per-key lineage graph (write frequency, hot keys) to exactly the §18-top-ranked node-side adversary — today sequential writes to one key are cryptographically *unlinkable* at nodes, and only same-lineage-point races are visible; (2) guards are cross-slot predicates and the only cross-slot linearization point is the fold's total order at finality — an accept-time verdict races the δ provisional window (a legal back-stamped op can still enter below the guarded op's fold position), so freezing effect at QC breaks §9/§11's layering and RESILIENCE §3.4's time-traveller containment ("durable ≠ applied" is the load-bearing price of offline writes and a skew window); (3) a node refusing on a witness makes an unprovable timing-dependent claim — honest quorum members legitimately disagree, reopening the frame-proofing the issuance chains closed and handing Byzantine nodes deniable censorship. The ZK invariant (§18) is hereby scoped to cover **content and undeclared structure**: the leakage boundary below is a *closed list*, the lineage linkage graph is inside the invariant, and any relaxation is a lane-3-grade design change. Two-tier guard vocabularies (node-checkable vs fold-only) are rejected with it — they fork verdict *timing* across guard kinds, §11's exact don't-conflate. The real knob for effect-latency is **δ** (§3.4: one knob, both directions); the GC cost of committed-but-rejected ops is already bounded ∝ churn·W by the conveyor.
...
Checkpoint {                          # control op — requires the `compact` capability (§15)
  ...
  state_acc:  ecmh(live state at cut)                   # audit anchor: ECMH accumulator over live (key,value,version,attempt) — ACCUMULATOR.md
...
- **No cut-lag window.** The cut is pinned at the finality frontier itself (`cut ≤ F`, next rule) — there is **no audit-window `W`**. The old rationale (lag the cut below `F` to give resident full-history clients time to recompute the audit anchor before nodes GC the evidence) is retired (ACCUMULATOR §2): the audit is deterministic recomputation of the `state_acc` accumulator against **client-durable lineage**, not a timed race, and silent omission is foreclosed *structurally* at the nodes (the `retained` partition, below). `W` is deleted, not tunable.
...
- **Derive-and-verify.** Bootstrap clients fold `retained ∘ tail` (bootstrap semantics below). A key-holder with continuity across the cut *derives* the same barrier state and **verifies** the checkpoint by recomputing the **`state_acc` ECMH accumulator** (ACCUMULATOR §5) — incrementally, O(Δ), never an O(n) snapshot. They agree whenever the checkpoint is honest; a mismatch is a corrupt checkpoint — a loud, portable audit failure, never something the fold silently adopts.
...
- **Bootstrap.** A new client pulls the retained set (sparse `PULL`s, verified per-author against the `retained` digests — an omission localizes to an author, not to a failed 10 GB fetch), folds the retained winners' **mutations only** in `(hlc, author, seq, op_hash)` order — **no guard re-evaluation** (it is reading settled state, not re-deciding CAS; the guards' truth was consumed at the original fold position, whose context is gone) — applies the `attempts` sidecar, verifies `state_acc`, then folds the tail normally above the barrier.
- **Trust surface — why trusting the checkpoint is sound (and smaller than it was).** Compaction **cannot alter a byte; it can only select.** Retained winners are the original author-signed envelopes: fabricating a value would require forging a client signature, so the compactor's entire power over sealed history is omission/selection among genuinely-authored ops — "a compression channel, never a write channel" is now structural, not merely audited. A bootstrap client *verifies*: genesis → the unbroken manager/compactor control chain → the checkpoint signature; every retained op's author signature, cert chain, and `(hlc, seq)` provenance; the `retained` digests; and the recomputed **`state_acc`** (ACCUMULATOR §5). What it *trusts* is exactly the **dead set** — that nothing live was omitted and nothing superseded was kept. Silent **omission** is foreclosed *structurally* at the storage nodes: `retained = covered ∖ dead` is checked against the signed `retained` commitment (baseline verification, below), so a node holding a dropped op refuses the checkpoint — no decryption, no window. The residual — a live winner wrongly marked `dead`, or a superseded op wrongly kept — is caught **opportunistically** by any continuity-holding key-holder recomputing the accumulator, against **client-durable lineage**: an author holds its ops + QCs indefinitely, so a committed op stays provable long after every node GC'd it, and detection therefore **never races GC**. A bad checkpoint is thus a *trusted-compactor compromise* — the compactor's declared blast radius above (wrongful-but-auditable GC): detected into portable evidence, ejected, recovered — never prevented, and with no `W`. Checkpoint-sync here costs zero trust relative to genesis-replay: genesis and checkpoint are signed by the same root, so replaying history would terminate in the same anchor (this is weak-subjectivity sync made free by a pre-existing root; history replay's only marginal value — catching manager misbehavior along the way — is provided by the residents' standing audit).
...
- **Hashes** are the hard one (content addresses everywhere): migration is a new chain grafted at a checkpoint fence, dual-hashing the retained-set commitments and state accumulators through the transition. Honest cost: a hash migration is a planned rebirth, not a rolling patch.
...
- The concrete value of the skew window δ (§9) — semantics fixed; **post-NOTES-52 (offline single-push struck), δ is a pure clock-skew bound: a small multiple of measured NTP dispersion (~1 s class)**; whether it is roster-op-settable per NOTES 32(a) remains open.
...
- ~~Checkpoint cadence policy~~ **Resolved at rev 6:** compaction is a continuous conveyor (§12) — slot-state GC, receipt floors, tombstone reclamation, and lane-2 fences all ride the routinely-advancing cut. ~~The remaining open constant is the cut-lag audit window **W**.~~ **Retired (ACCUMULATOR §2):** `W` is deleted — the cut is pinned at `F` itself, the audit is deterministic accumulator recomputation against client-durable lineage (no timed race), and silent omission is foreclosed structurally at the nodes. The only surviving compaction tunable is checkpoint *cadence* (node-local, liveness-only).
...
| `δ` (skew window) | control plane (lane 3, NOTES 32) | clock-skew tolerance (offline-viability struck, NOTES 52 — δ is skew-only, ~1 s class) | the time-traveller's playground; finality latency (floors trail `now − δ`); the provisional-flip window |
...
**Trilemma honesty.** The surface does not collapse into one trilemma; it is mostly two-way dials, with **one** genuine three-way tension: the **replication trilemma** — durability × footprint (disk + gossip) × cluster cost, all riding `n`, where you pick the point on the line but never leave it. (Compaction *was* a second trilemma — disk floor × bandwidth × audit window — but retiring `W` (ACCUMULATOR §2) collapses its third axis: it is now a plain two-way dial, disk floor × compaction bandwidth via cadence, GC'ing sooner costing only bigger/more-frequent deltas.) Two non-tensions worth naming so they aren't traded by accident: durability and partition tolerance are the **same dial** (`n` — they move together, never against each other), and write availability under partition is not a dial at all — writes park on any minority side by construction (quorum commitment) while reads continue everywhere (local fold, SEC); that corner of CAP was chosen in §1 and is not for sale.
