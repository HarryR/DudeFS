...
- **Scale:** 1–3 clients on average; small total data size. Not built for high write concurrency.
...
|---|---|---|
| **Manager** | Fully trusted (root). Compromise = game over; nothing is defended below it. | Trust anchor. Signs authorization certs for clients and nodes. Owns the server/node roster (roster changes are signed control ops). Mints baselines (compaction). Just another client, with privileged capabilities. |
...
The **manager is just another client** whose ops (roster changes, baselines, cert issuance/revocation) are privileged because they're signed by the root key. Nothing in the data path waits on the manager being online.
...
- Dueling proposers can livelock in theory; with 1–3 clients, randomized backoff suffices (noted, accepted).
...
## 12. Compaction (baselines)

Compaction is a **rewrite, not a deletion** — nothing expires; only intermediate *history* below the horizon is discarded, never a live value.

```
Checkpoint {                          # control op, signed by the MANAGER ONLY
  cut:        { author_i: (seq_i, head_hash_i), ... }   # frontier being sealed — must be entirely FINAL (§9)
  state_root: merkle_root(folded state at cut)          # makes compaction auditable
  snapshot:   ciphertext(materialized state at cut)     # new-client bootstrap base; includes per-key (version, attempt)
  keyepoch, hlc, prev(manager-chain), sig(manager)
}
```

- **The cut must be final before it is sealed.** The manager checkpoints only at a frontier covered by quorum watermarks — so no committed-but-unseen op can ever sort below the cut. This is what makes the next rule sound:
- **The barrier sits immediately above the cut — not at the checkpoint op's own fold position.** Because the cut is entirely final, every op it covers sorts below every op that can still commit (finality bounds their `hlc`s), so the fold is well-defined as: fold the covered set, apply the barrier, fold the tail in total order. The checkpoint op itself is an ordinary control op in the tail; only its *pinned cut* places the barrier. Ops that commit while the manager is minting the checkpoint (`hlc` above the cut's finality frontier but below the checkpoint op's `hlc`) are tail ops and fold **above** the barrier — for everyone.
- **Derive-and-verify.** Bootstrap clients fold `snapshot ∘ tail`. Full-history clients *derive* the same barrier state from raw history — applying tombstone-death for keys dead at-or-below the cut — and **verify** the checkpoint by recomputing `state_root`. The two agree byte-for-byte whenever the checkpoint is honest; a mismatch is a corrupt checkpoint — a loud, portable audit failure (see "compaction can't tamper" below), never something the fold silently adopts.
- **Tombstones die at the barrier — and so does attribution to dead keys.** The snapshot materializes live keys only: a key deleted below the cut is absent from it, and its lineage restarts at `(⊥, 0)`; likewise a never-valued key whose `(⊥, n)` lineage was only ever attempt-advanced resets to `(⊥, 0)`. The **key universe resets with it**: above the barrier, a key is attributable only if it is live at the barrier or named by an op above the cut — otherwise snapshot clients (who cannot decrypt sealed history) and full-history clients would attribute differently. Consequence: a CAS computed against a tombstone's version that commits *after* the checkpoint folds as `stale` — retry against the fresh lineage. Rare, safe, bounded by checkpoint cadence. Tombstones written *above* the cut survive to the next checkpoint.
- **Only the manager can authorize physical GC.** Nodes drop ops `≤ cut` (and slot acceptor-state for slots consumed `≤ cut`, per §8) only after seeing a *quorum-committed* manager checkpoint; they keep the checkpoint, the snapshot, and the pinned head hashes so later ops still `prev`-validate across the boundary. After GC, nodes refuse receipts for any op with `hlc` below the checkpoint horizon (a floor the watermark rule already implies, restated as a backstop).
- **Rogue-delete protection without a retention window:** a rogue client can only write tombstones — visible (signed, attributed) and reversible in the live log. Only the manager mints baselines, so deletions never become permanent except by manager action. No time-based expiry exists (this is a config dictionary; values do not age off).
- **Compaction can't tamper — it's auditable.** `state_root` lets any online client recompute the fold to the cut and confirm the manager's snapshot matches. Compaction is a *compression* channel, not a *write* channel.
...
- The live roster is learned by replaying **manager-signed roster ops** (control plane) from the log. New clients bootstrap from the latest **checkpoint snapshot** + tail ops, folding at the checkpoint barrier (§12); the snapshot carries per-key `(version, attempt)` so slot lineages continue seamlessly.
...
- The manager issues **capability certs**: `authz(subject_pubkey, capabilities, epoch)`, signed by the root key. Capabilities at minimum: `write` (client), `store` (node, requires BLS proof-of-possession — §3), and manager-privileged `compact` / `manage-roster` / `issue-revoke`.
...
- Because the snapshot seals the old fold's *output*, implementations may eventually delete old fold code entirely. **Compaction is the upgrade mechanism.**
...
- **Hashes** are the hard one (content addresses everywhere): migration is a new chain grafted at a checkpoint fence, dual-hashing the snapshot roots through the transition. Honest cost: a hash migration is a planned rebirth, not a rolling patch.
...
- Key-rotation mechanics: the re-wrapping flow, distribution of the epoch-key history to new clients, and whether snapshots re-encrypt under the current epoch.
...
- Checkpoint cadence policy: when the manager *should* mint baselines (size/age heuristics), given that slot-state GC, receipt floors, tombstone reclamation, and now lane-2 version fences all ride on it.
...
- **AEAD staging** (see IMPLEMENTATION.md): the POC launches with an `auth-only` payload suite — authenticated, *unencrypted*; zero-knowledge is loudly suspended while every other property (provenance, durability, CAS, finality) is exercised for real. Encryption arrives later as a **key rotation to a new suite** (per-`keyepoch` suites make the migration mechanism the existing one): a BLAKE2 stream+MAC construction or vendored ChaCha20-Poly1305. Slot tags stay PRF'd (keyed BLAKE2) in *all* suites, so slot logic never varies.
