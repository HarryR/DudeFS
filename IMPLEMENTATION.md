...
- **Artifact schemas:** field-for-field from DESIGN §5 (envelope), §8 (receipt: `op_hash ‖ config_epoch ‖ ballot`; QC: `{op_hash, config_epoch, ballot, signer_bitmap, sigs}`), §9/PROTOCOL §1 (frontier bundle: `heads ‖ checkpoint_head ‖ config_epoch ‖ floor`, one signature), §12 DESIGN (checkpoint: `cut, state_root, dead, retained, attempts` — rev 6), PROTOCOL §7.1 (endpoint record). Ballot = `(round:int, priority:bytes)` with `priority = h(slot_tag ‖ client_fp)` (DESIGN §8, NOTES 24d), ordered lexicographically; `hlc = (wall_ms:int, counter:int)`. Exact layouts live in `artifacts.py` + golden vectors, not re-specified here.
