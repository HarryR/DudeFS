...
> **Status:** the hand-off plan for an implementing agent. Normative sources: [DESIGN.md](DESIGN.md) (rev 5) · [PROTOCOL.md](PROTOCOL.md) · [ARCHITECTURE.md](ARCHITECTURE.md) · [MANAGER.md](MANAGER.md). Verification targets: [FORMAL.md](FORMAL.md). **If code and documents disagree, the documents win — or the discrepancy is raised as a doc change, never silently coded around.**
>
...
- **Artifact schemas:** field-for-field from DESIGN §5 (envelope), §8 (receipt: `op_hash ‖ config_epoch ‖ ballot`; QC: `{op_hash, config_epoch, ballot, signer_bitmap, sigs}`), §9/PROTOCOL §1 (frontier bundle: `heads ‖ checkpoint_head ‖ config_epoch ‖ floor`, one signature), §11 DESIGN (checkpoint), PROTOCOL §7.1 (endpoint record). Ballot = `(round:int, client_fingerprint:bytes)`, ordered lexicographically; `hlc = (wall_ms:int, counter:int)`. Exact layouts live in `artifacts.py` + golden vectors, not re-specified here.
...
- **M5 — control plane.** Certs, fold-positional revocation, roster 1→3→5 with possession barrier + learner catch-up, the public roster slot (two competing managers → one activation — B4), `RERECEIPT` across the epoch bridge (B5).
- **M6 — compaction.** Checkpoint mint, lazy GC, snapshot-bootstrap ≡ full-history fold (A4 as an integration test), tombstone death at the barrier, receipt floor at the horizon.
...
4. **Adversarial builds** — small subclass overrides creating an equivocating acceptor, a floor perjurer, a time-traveller client; assert containment + evidence generation (RESILIENCE §3.1's table as tests).
